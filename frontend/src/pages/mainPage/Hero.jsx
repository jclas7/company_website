...
function Hero(props) {
  return (
    <div className="relative min-h-[110vh] bg-gradient-to-b from-blue-400 to-white pb-0">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
          <div className="flex-1 text-center lg:text-left">
            ...
            <p className="text-lg sm:text-xl text-gray-800 text-semibold mb-8 max-w-2xl mx-auto">
              안전하고 효율적인 태양광 설비 설치부터 유지보수까지, 전문들이 함께
              합니다.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <button
                className="px-8 py-4 bg-blue-600 text-white rounded-lg hover:bg-blue-900 transition-colors duration-300
            text-lg font-semibold shadow-lg hover:shadow-xl"
              >
                상담 신청하기
              </button>
              <button
                className="px-8 py-4 bg-white text-blue-600 rounded-lg border-2 border-blue-600 hover:bg-blue-50 transition-colors duration-300 text-lg
            font-semibold"
              >
                더 알아보기
              </button>
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto">
          {[
            { number: "1,200+", lable: "설치 완료" },
            { number: "98%", lable: "고객 만족도" },
            { number: "15년", lable: "업계 경력" },
            { number: "24/7", lable: "기술 지원" },
          ].map((stat, index) => (
            <div key={index} className="text-center">
              <div className="text-3xl font-bold text-blue-600">
                {stat.number}
              </div>
              <div className="text-gray-900">{stat.lable}</div>
            </div>
          ))}
        </div>
      </div>
